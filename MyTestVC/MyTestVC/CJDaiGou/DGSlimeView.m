...
- (void)initWithSupperView:(UIView *)backView withTuoZhuaiBlock:(TuoZhuaiBlock)block{
    self.shapLayer.frame = CGRectMake(0, 0, CGRectGetWidth(backView.frame), CGRectGetHeight(backView.frame));
    self.headDot.center = self.center;
    self.trailDot.center = self.center;
    self.tuoZhuaiBlock = block;
    [backView.layer addSublayer: self.shapLayer];
    [backView addSubview: self.trailDot];
    [backView addSubview: self.headDot];    // 注意加入顺序
    
    
    _num_label = [[UILabel alloc] initWithFrame:self.headDot.bounds];
//    _num_label.text = @"0";
    ...
    [self.headDot addSubview:_num_label];
...
// 通过字符串判断label的宽度。。。
- (CGSize)sizeOfString:(NSString *)str withFont:(UIFont *)font withHigh:(float)highth
{
    CGSize bubbleSize =CGSizeZero;
    CGSize size = CGSizeMake([UIScreen mainScreen].bounds.size.width,highth);
#if (defined(__IPHONE_OS_VERSION_MIN_REQUIRED) && __IPHONE_OS_VERSION_MIN_REQUIRED >= 70000)
    NSDictionary *stringAttributes = [NSDictionary dictionaryWithObject:font forKey: NSFontAttributeName];
    bubbleSize =  [str  boundingRectWithSize:size options:(NSStringDrawingUsesFontLeading | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingTruncatesLastVisibleLine) attributes:stringAttributes context:nil].size;
#else
